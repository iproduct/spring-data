...
public class DiabloGames {
    public static void main(String[] args) {
        ...
        try (Connection con = DriverManager.getConnection(
            ...
            DSL.using(con)
                    .fetch(props.getProperty("sql.games"), username)
                    .stream()
                    .map(recordMapper -> recordMapper.get(recordMapper.field("first_name"))
                            + " " + recordMapper.get(recordMapper.field("last_name"))
                            + ": " + recordMapper.get(recordMapper.field("name")))
